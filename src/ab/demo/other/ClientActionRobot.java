...
public class ClientActionRobot {
	...
	public int[] returnScreenShot(){
		try {
			...
			byte[] bytebuffer;
			int[] returnbuffer = new int[totalBytes];
			...
			for (int y = 0; y < height; y++) {
				for (int x = 0; x < width; x++) {
					returnbuffer[y * width + x] = imgbyte[(y * width + x) * 3] & 0xff;
					returnbuffer[width * height + y * width + x] = imgbyte[(y * width + x) * 3 + 1] & 0xff;
					returnbuffer[2 * width * height + y * width + x] = imgbyte[(y * width + x) * 3 + 2] & 0xff;
				}
			}
