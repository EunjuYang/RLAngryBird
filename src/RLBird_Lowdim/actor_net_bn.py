...
class ActorNet_bn:
    ...
    def __init__(self, num_states, num_actions, BATCH_SIZE):
        ...
        with self.g.as_default():
            ...
            self.W3_a = tf.Variable(tf.random_uniform([N_HIDDEN_2, num_actions], 1, 5))
            self.B3_a = tf.Variable(tf.random_uniform([num_actions], 5, 10))
            ...
            self.t_B2_a = tf.Variable(
                tf.random_uniform([N_HIDDEN_2], -1 / math.sqrt(N_HIDDEN_1), 1 / math.sqrt(N_HIDDEN_1)))
            self.t_W3_a = tf.Variable(tf.random_uniform([N_HIDDEN_2, num_actions], 1, 3))
            self.t_B3_a = tf.Variable(tf.random_uniform([num_actions], 5, 10))
