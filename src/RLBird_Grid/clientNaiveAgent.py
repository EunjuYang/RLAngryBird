...
class NaiveAgent:
    ...
    def getScreenBuffer(self,width,height,buffer):
        """
    ...
    def solve(self):
        ...
        if sling != None:
            ...
            if len(pigs) != 0:
                ...
                self.ar.fullyZoomOut()
                screenshot = self.ar.doScreenShot()
                ScreenBuf = self.getScreenBuffer(840,480,screenshot)
                #print ScreenBuf
                vision = Vision(screenshot)
                ...
                if _sling != None :
                    ...
                    if scale_diff < 25:
                        ...
                        print "## releasePoint : (%d, %d)" % (releasePoint.getX(),releasePoint.getY())
                        print "## refPoint : (%d, %d)" % (refPoint.x,refPoint.y)

                        if dx < 0:
                            self.ar.shoot(int(refPoint.x), int(refPoint.y), int(dx), int(dy), 0, int(tapTime), False)
                            state = self.ar.checkState()
                            scores = self.ar.checkMyScore()
                            print "## score %d" % scores[self.currentLevel - 1]
